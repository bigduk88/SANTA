...
public class RegistrationService {
    ...
    @Transactional
    public ResponseEntity mekeRegistration(RegisterRequestDto reqisterRequestDto, Long board_id, UserDetailsImpl userDetails) {
        ...
        if (registration != null **registration.getAccount().equals(account)){
            return new ResponseEntity<>("중복요청은 안됩니다.", HttpStatus.BAD_REQUEST);
        }

        Registration registration = Registeration.builder()
    ...
    @Transactional
    public ResponseEntity declineRegistration(Long board_id, Long register_id, UserDetailsImpl userDetails) {
        Board board = boardRepository.findById(board_id).orElse(null);
        if (board == null) {
            return  ResponseEntity.badRequest().body("없는 게시판입니다");
        }
        Registration registration = registrationRepository.findById(userDetails.getUsername()).orElse(null);
        ...
        if (registration.getCreatedBy().equals(account.getNickname())) {
            registrationRepository.deleteById(register_id);
            return RespponseEntity.ok().body("요청을 취소 하였습니다");
        }
        return ResponseEntity.ok().body("취소는 주취자 또는 신청자 봉닌만 가능합 니다.");
    }
}
